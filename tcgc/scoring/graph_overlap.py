...
def _edge_match_score(gold_edges: list[dict[str, Any]], pred_edges: list[dict[str, Any]]) -> float:
    ...
    for e in gold_edges:
        gold_by_pair.setdefault(_edge_key(e), []).append(e.get("type", ""))
    for e in pred_edges:
        pred_by_pair.setdefault(_edge_key(e), []).append(e.get("type", ""))
...
def score(
    ...
    *,
    ...
    notes: list[str] = []
    if not gold.get("edges"):
        notes.append("gold has no edges — score reduces to node overlap")
    if not pred.get("edges") and gold.get("edges"):
        notes.append("pred has no edges — edge_jaccard is 0")
    return ScoreResult(
        value=value, components={"node_jaccard": node_j, "edge_jaccard": edge_j}, notes=notes
    )
